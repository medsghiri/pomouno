...
export function TaskCompletionDialog({
    ...
    const getTaskTypeIcon = () => {
        if (task.spacedRepetition?.enabled) return '🧠';
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-lg w-[95vw] max-w-[95vw] sm:w-full">
                ...
                <div className="space-y-4">
                    ...
                    {task.estimatedSessions > 0 && !task.spacedRepetition?.enabled && !task.recurring?.enabled && (
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                <span>Progress</span>
                                <span>{(todaysTaskSessions || 0)} / {task.estimatedSessions} sessions</span>
                            </div>
                            <div className="w-full bg-accent rounded-full h-2">
                                <div
                                    className="bg-gradient-to-r from-red-500 to-orange-500 h-2 rounded-full transition-all duration-300"
                                    style={{
                                        width: `${Math.min(((todaysTaskSessions || 0) / task.estimatedSessions) * 100, 100)}%`
                                    }}
                                />
                            </div>
                        ...
                        <p className="text-lg font-medium text-foreground mb-2">
                            {task.spacedRepetition?.enabled
                                ...
                                : task.recurring?.enabled
                                    ...
                                    : task.estimatedSessions > 0 && (todaysTaskSessions || 0) >= task.estimatedSessions
                            ...
                            {task.spacedRepetition?.enabled
                                ...
                                : task.recurring?.enabled
                                    ...
                                    : task.estimatedSessions > 0 && (todaysTaskSessions || 0) >= task.estimatedSessions
                                        ...
                                        : task.estimatedSessions > 0 && (todaysTaskSessions || 0) < task.estimatedSessions
                                            ? `You've completed ${(todaysTaskSessions || 0)} of ${task.estimatedSessions} estimated sessions.`
                    ...
                    {task.spacedRepetition?.enabled && (
                        <div className="text-xs text-muted-foreground text-center p-2 bg-red-50 dark:bg-red-900/20 rounded">
                            💡 Spaced repetition will schedule your next review based on how well you know this item.
                        </div>
                    )}
