...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    const handleUserLogin = async (newUser: User) => {
        try {
            ...
            toast({
                title: "Sync issue",
                description: "Some data may not have synced properly. Your local data is safe.",
                variant: "destructive",
            });
        }
    };
    ...
    const logout = async () => {
        try {
            ...
            toast({
                title: "Signed out",
                description: "You've been signed out. Your basic timer functionality is still available.",
            });
        } catch (error) {
...
export function useAuthUser(): { user: User | null; loading: boolean; error: Error | undefined } {
    const { user, loading, error } = useAuth();
    return { user, loading, error };
}

export function useFeatureAccess(feature: Feature) {
    const { canAccessFeature, requiresAuth, getFeatureAccessMessage, showUpgradePrompt, showSignInPrompt } = useAuth();

    return {
        canAccess: canAccessFeature(feature),
        requiresAuth: requiresAuth(feature),
        accessMessage: getFeatureAccessMessage(feature),
        showUpgradePrompt: (context?: string) => showUpgradePrompt(feature, context),
